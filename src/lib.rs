...
/*
/// Builds a page table entry (PTE) or page directory entry (PDE) from a physical address and flags.
#[inline(always)]
pub const fn make_page_entry(phys_addr: u32, flags: u32) -> u32 {
    (phys_addr & page::ADDR_MASK) | (flags & !page::ADDR_MASK)
}

/// Extracts the physical address from a PTE or PDE.
#[inline(always)]
pub const fn extract_phys_addr(entry: u32) -> u32 {
    entry & page::ADDR_MASK
}
...
│   │   ├── mod.rs
│   │   ├── registers/            # ax, bx, cx, dx, si, di, bp, sp, ip, cs, ds, es, ss, flags
│   │   ├── intrinsics/           # cli, sti, hlt, nop, wait, stc, clc, cmc, cld, std
│   │   ├── stack/                # push, pop
│   │   ├── control/              # jump_near, jump_far, ret_near, ret_far
│   │   ├── interrupts/           # int, iret, InterruptFrame
│   │   ├── io/                   # inb, outb, inw, outw
│   │   └── far/                  # FarPtr, read_byte, write_byte, read_word, write_word
│   │
│   ├── protected/                # 32-bit protected mode
│   │   ├── mod.rs
│   │   ├── registers/            # eax, ebx, ecx, edx, esi, edi, ebp, esp, eip, eflags, segment selectors
│   │   ├── intrinsics/           # cli, sti, hlt, nop
│   │   ├── stack/                # push, pop (u32)
│   │   ├── control/              # jump_near, jump_far, ret_near, ret_far
│   │   ├── interrupts/           # int, iret, InterruptFrame (32-bit)
│   │   ├── io/                   # inb, outb, inw, outw, inl, outl
│   │   ├── gdt/                  # GDT descriptors, lgdt, sgdt
│   │   ├── idt/                  # IDT descriptors, lidt, sidt
│   │   ├── ldt/                  # LDT descriptors, lldt, sldt
│   │   ├── tss/                  # TSS, ltr, str
│   │   ├── paging/               # Page tables, invlpg, cr3
│   │   └── cpuid/                # CPUID instruction
│   │
│   └── long/                     # 64-bit long mode
│       ├── mod.rs
│       ├── registers/            # rax, rbx, rcx, rdx, rsi, rdi, rbp, rsp, rip, rflags, r8-r15
│       ├── intrinsics/           # cli, sti, hlt, nop, swapgs
│       ├── stack/                # push, pop (u64)
│       ├── control/              # jump_near, jump_far, ret_near, ret_far
│       ├── interrupts/           # int, iret, InterruptFrame (64-bit)
│       ├── io/                   # inb, outb, inw, outw, inl, outl
│       ├── gdt/                  # GDT descriptors (64-bit), lgdt, sgdt
│       ├── idt/                  # IDT descriptors (64-bit), lidt, sidt
│       ├── ldt/                  # LDT descriptors, lldt, sldt
│       ├── tss/                  # TSS (64-bit), ltr, str
│       ├── paging/               # 4-level/5-level paging, invlpg, cr3
│       ├── cpuid/                # CPUID instruction
│       ├── msr/                  # rdmsr, wrmsr, MSR constants
